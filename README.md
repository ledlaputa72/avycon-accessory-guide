...
## 🖼️ 이미지 온라인 저장 (Vercel Blob) — 공유됨
업로드한 제품 이미지는 **Vercel Blob(공개 스토어)** 에 저장되어 **공개 URL**을 받고, 그 URL이 **CSV의 `ImageURL` 컬럼**으로 함께 나갑니다. 따라서 한 사람이 이미지를 올리고 CSV를 내보내면, **다른 팀원이 그 CSV를 가져왔을 때 동일한 이미지가 보입니다.**

흐름: `업로드 → /api/upload(서버리스) → 공개 URL → CSV ImageURL → 다른 사용자 가져오기 → 동일 이미지`
- 업로드 전 클라이언트에서 1280px·JPEG로 자동 축소.
- API/스토어 미설정 또는 로컬 미리보기 시에는 base64로 자동 폴백(그 브라우저에서만 보임).

**최초 1회 설정(Vercel):** Storage → **Public** Blob 스토어 생성 → 프로젝트 Connect 시 **"Add a read-write token env var" 체크**(접두사 `BLOB`) → 재배포. 자세한 건 `CLAUDE.md` 참고.

## ⚠️ 알아둘 한계 (정적 사이트라서)
- **제품 데이터는 브라우저 localStorage에 저장** → 추가한 제품은 **그 브라우저에서만** 보입니다. 팀원끼리 자동 공유는 안 되며, **CSV 내보내기/가져오기로 동기화**합니다. (이미지는 위처럼 URL로 공유됨)
